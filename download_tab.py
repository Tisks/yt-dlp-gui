...
Pulling this out of gui.py is the first step toward running more than one
batch at once -- everything a batch owns (rows, processes, output/error text)
now lives on its own object instead of being flat attributes on the app.
"""
...
class DownloadTab:
    """A single batch. `frame` is the widget the app packs into its window."""
    ...
    def _add_url_row(self, initial_url=""):
        ...
        # The first row is the batch's permanent URL field; only rows added on
